...
- Review의 Save 상태는 `Pending / Saving / Saved / Retry`가 색과 미세한 강조 애니메이션까지 함께 보이게 유지해, 자동저장 흐름을 표 안에서 바로 읽게 유지한다.
- Review 하단 액션 바에는 저장 실패 row가 있을 때 바로 그 행으로 점프하는 shortcut을 함께 보여줘, retry 대상을 다시 찾는 시간을 줄인다.
- Review에서 저장 실패가 여러 건이면 하단에서 한 번에 재시도할 수 있게 유지해, 사용자가 row별 retry를 반복 클릭하지 않게 한다.
...
- Review의 대량 작업은 통화뿐 아니라 Category도 `선택 / 전체` 기준으로 한 번에 맞출 수 있게 유지해, OCR 후 반복 분류 수정 시간을 줄인다. 관련 파일: `templates/report_wizard_review.html`
- Finalize 제목은 문서정책 preset과 연결된 `Use suggested title` 보조 액션을 유지해, 문서 유형을 바꾼 뒤 필수 입력을 더 빨리 채우게 한다.
- Finalize 기간 입력은 선택 영수증 날짜 범위가 이미 있으면 `Use receipt period`로 바로 채울 수 있게 유지해, Step 3에서 기간을 다시 손으로 맞추는 비용을 줄인다. 관련 파일: `templates/report_wizard_generate.html`
...
- Template Preview compare에는 짧은 범례 chip 외에도 한 줄 설명을 두어, 왼쪽 highlighted cell이 실제 override 결과라는 점을 바로 이해하게 유지한다.
- Template Preview compare의 diff cell은 배경색만이 아니라 경계선/굵기까지 같이 강조해, 작은 셀에서도 변경 위치가 바로 보이게 유지한다.
...
- Template Preview compare는 필요할 때 `Changed sheets only`로 바뀐 시트만 먼저 보게 유지해, override 영향 시트가 많지 않은 경우 롤아웃 점검 시간을 줄인다. 관련 파일: `templates/admin.html`
